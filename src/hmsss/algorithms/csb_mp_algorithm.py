...
def create_alphabet(hash_table, redundancy_hash, q=2):
    ...
    for key, lst in hash_table.items():
        for item in lst:
            element_counts[item] = (
                element_counts.get(item, 0) + redundancy_hash[key]
            )  # bei redundz minimierten darf das  hier kein feste wert sein, sondern muss der anzahl der reduzierten gene_cluster entsprechen
...
def initialize_Instances(
    Pattern, current_Seq_key, Seq_hash, InstanceListP, MatchLists, NextMatch
):
...
def findInstances(
    ...
    for key, lst in Seq_hash.items():
        if key != current_Seq_key and key in expanding_set:
            ...
            sigma = Pattern[-1]  # Letztes Zeichen von P
            MatchList = MatchLists[sigma][key]
            add_flag = extendInstances(
                Pattern, key, InstanceListP, MatchList, NextMatch, k, q
            )  # extends a csb in the list returns 1 if successfull
            if add_flag:
                extended_keys.add(key)
    return extended_keys
...
def extendInstances(Pattern, key, InstanceListP, MatchList, NextMatch, k, q):
    ...
    for index, chain_tuple in enumerate(instanceListP_y):
        ...
        if index + 1 < len(
            instanceListP_y
        ):  # find the next csb in the chain or set infinite
        ...
        for i in range(0, length):
            ...
            for j in range(i + 1, length + 1):
                ...
                if pattern_tuple in computed_Instances_dict.keys():
                    ...
                    initilized = initialize_Instances(
                        pattern,
                        key,
                        gene_clusters,
                        InstanceListP,
                    ...
                    if count > q:
                        discontinued = (
                            active_expanding_keys - extended
                        )  # returns the keys of gene clusters that were not extended
                        active_expanding_keys = (
                            active_expanding_keys & extended
                        )  # returns the keys of the gene clusters that were extended and will be tested in the next round
