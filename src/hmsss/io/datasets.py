...
def main_binary_dataset(options, directory, protein_dict, cluster_dict, taxon_dict):
    ...
    protein_binary_matrix_dict, prot_headers = create_presence_absence_matrix(
        taxon_dict, options.fetch_proteins, protein_presence_dict
    )
    process_binary_dataset(
        taxon_dict,
        protein_binary_matrix_dict,
        prot_headers,
        directory,
...
def process_binary_dataset(
    taxon_dict, binary_matrix_dict, headers, directory, divide_sign, suffix=""
):
    """
    ...
    taxon_group_counts_by_level = count_taxon_group_presence_by_level(
        taxon_dict, binary_matrix_dict, divide_sign
    )

    # Write the taxon group counts to TSV files
    write_taxon_group_counts_to_tsv(
        taxon_group_counts_by_level, directory, headers, suffix
    )
...
def create_presence_absence_matrix(
    taxon_dictionary, order_list, genome_presence_dictionary
):
...
def count_taxon_group_presence_by_level(taxon_dict, presence_absence_dict, divide_sign):
    ...
    for genomeID, presence_list in presence_absence_dict.items():
        ...
        lineage = taxon_dict[genomeID].split(
            divide_sign
        )  # Split the lineage into components
        ...
        for level, taxon_group in enumerate(lineage[1:]):
            ...
            taxonomic_rank = (
                taxonomic_ranks[level]
                if level < len(taxonomic_ranks)
                else f"Level {level + 1}"
            )
            ...
            if not taxon_group_counts_by_level[taxonomic_rank][taxon_group]:
                taxon_group_counts_by_level[taxonomic_rank][taxon_group] = [0] * len(
                    presence_list
                )
...
def write_taxon_group_counts_to_tsv(
    taxon_group_counts_by_level, output_directory, headers, suffix
):
    """
    ...
    for taxonomic_rank, taxon_group_counts in taxon_group_counts_by_level.items():
        ...
        output_file = os.path.join(
            output_directory, f"Absolute_counts_{suffix}_{taxonomic_rank}.tsv"
        )
...
def iTol_taxonomy_range_dataset(directory, taxon_dict):
    ...
    for genomeID in taxon_dict.keys():
        ...
        string = (
            taxon_dict[genomeID] + "\t2" + "\t10" + "\t#ff0000" + "\t1" + "\t1" + "\n"
        )
        writer.write(string)
...
def dataline_combinations(dataline_dict, headers, filepath, combine=5, min_combine=1):
    ...
    for genomeID, dataline in dataline_dict.items():
        res = dict(
            zip(headers, dataline)
        )  # mapping the headline to the value for the dataline of a single genome
...
def dataset_statistics(database, directory, headers, dataline_dict):
    ...
    for lin in lineage:
        ...
        order_of_taxonomy = list(
            dict.fromkeys(order_of_taxonomy)
        )  # makes each lineage unique
        ...
        for taxon in order_of_taxonomy:
            ...
            try:
                writer.write(
                    "\t".join(
                        str(int(item) / int(id_count)) for item in taxon_sum[taxon]
                    )
                )
            except:
    ...
    with sqlite3.connect(database) as con:
        ...
        for record in SeqIO.parse(filepath, "fasta"):
            ...
            dataset_range_line = (
                ...
                else f"{lineage}{trennzeichen}{record_dict[genomeID]}"
            )

            # Fetch protein domains
            # Creating the actual range dataset infos
            cur.execute(
                """SELECT domain FROM Domains WHERE proteinID = ?""", (proteinID,)
            )
            domain_string = ""
            for row in cur:
                result_str += row[0] + "-"
    ...
    with sqlite3.connect(database) as con:
        ...
        for record in SeqIO.parse(filepath, "fasta"):
            ...
            dataset_range_line = (
                lineage
                if record_dict[genomeID] == 1
                else str(lineage) + str(trennzeichen) + str(record_dict[genomeID])
            )
            ...
            cur.execute(
                """SELECT clusterID from Proteins WHERE proteinID = ?""", (proteinID,)
            )
            row = cur.fetchone()
            if not row:
                continue
            ...
            )  # sort proteins into correct order
