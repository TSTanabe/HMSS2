...
following extensions: %s" % ' '.join(valid_extensions.keys()))
    ...
    def set_euk_hmm(self, args):
        ...
        if hasattr(args, 'euk_hmm_file'):
            ...
            # pip.
            setattr(args, 'euk_hmm_file',
                    os.path.join(os.path.dirname(inspect.stack()[-1][1]), '..', 'share', '18S.hmm'))
        else:
    ...
    def parameter_checks(self, args):
        ...
        if args.subparser_name == 'graft':
            ...
            if float(args.placements_cutoff) < float(0.5) or float(args.placements_cutoff) > float(1.0):
                logging.info(
                    'Please specify a confidence level (-d) between 0.5 and 1.0! Found: %s' % args.placements_cutoff)
                exit(1)
    ...
    def set_attributes(self, args):
        ...
        if args.graftm_package:
            ...
            if args.search_method == self.DIAMOND_SEARCH_METHOD:
                ...
                raise Exception("Specified DIAMOND databases when not using the diamond search pipeline. Using: %s" % (
                    args.search_method))
            ...
            if args.search_method == self.HMMSEARCH_SEARCH_METHOD:
                ...
                raise Exception("Specified HMM search_hmm_files when not using the hmmsearch pipeline. Using: %s" % (
                    args.search_method))
