...
def concat_alignments(options):
    ...
    filepaths_sorted = sorted(
        filepaths, key=lambda x: x
    )  # Replace with custom order if needed
    ...
    for filepath in filepaths_sorted:
        ...
        for record in SeqIO.parse(filepath, "fasta"):
            ...
            if genomeID in genomeIDs_in_file:
                ...
                print(
                    f"WARNING: Encountered doublicates of {genomeID} in {filepath}. Removed from concatenation."
                )
                continue
            else:
    ...
    for filepath in filepaths_sorted:
        ...
        for absent in absent_genomes:
            if options.gaps:
                print(
                    f"WARNING: Missing sequence for {absent} in {filepath}. Concatenating gaps instead."
                )
                concat_dict[absent] += length * "-"
            else:
                print(
                    f"WARNING: Missing sequence for {absent} in {filepath}. Removing concatenated sequence."
                )
...
def get_alignments(options):
    ...
    if not alignment_files:
        ...
        for fasta_file in fasta_files:
            ...
            base_name = os.path.splitext(os.path.basename(fasta_file))[
                0
            ]  # Extract the filename without the .faa extension
            output_fasta = os.path.join(
                input_dir, f"{base_name}.fasta_aln"
            )  # Create the output filename with .aln extension in the same directory as the input file
...
def align_fasta_with_mafft(input_fasta, output_fasta):
    ...
    mafft = myUtil.find_executable(
        "mafft"
    )  # Ensure this function finds the MAFFT executable
    ...
    try:
        with open(output_fasta, "w") as output_file:
            ...
            subprocess.run(
                [mafft, "--thread", "2", "--auto", input_fasta],
                stdout=output_file,
        ...
        print(
            "MAFFT executable not found. Please make sure MAFFT is installed and in your system's PATH."
        )
...
def add_genomic_context(database, filepath):
    ...
    protein_ids = [
        record.id.split("-", 1)[-1] for record in SeqIO.parse(filepath, "fasta")
    ]
    ...
    with sqlite3.connect(database) as con:
        ...
        if cluster_ids:
            ...
            for row in rows:
                ...
                if domain:
                    context_data[proteinID]["domains"].append(
    ...
    with open(filepath + "_gene_vicinity", "w") as writer:
        ...
        for proteinID, clusterID in cp_dict.items():
            ...
            domains = ";".join(
                [
                    f"{d[0]}({d[1]}-{d[2]}):{d[3]}"
                    for d in protein_data.get("domains", [])
                ]
            )
            taxon_data = fetch_taxonomy(
                database, genomeID
            )  # Fetch taxonomy data for genomeID

            writer.write(
                f"0\t{proteinID}\t{domains}\t{protein_data.get('sequence', '')}\t{protein_data.get('contig', '')}\t{protein_data.get('start', '')}\t{protein_data.get('end', '')}\t{protein_data.get('strand', '')}\t"
            )
...
def fetch_taxonomy(database, genomeID):
    ...
    with sqlite3.connect(database) as con:
        cur = con.cursor()
        cur.execute(
            "SELECT Superkingdom, Clade, Phylum, Class, Ordnung, Family, Genus, Species FROM Genomes WHERE genomeID = ?",
            [genomeID],
        )
        row = cur.fetchone()
        return (
            row if row else [""] * 8
        )  # Return empty fields if no taxonomy data is found
...
def sort_gene_vicinity(input_file, output_file):
    ...
    sorted_rows = sorted(
        rows, key=lambda x: tuple(x[header] for header in sort_headers)
    )
...
def help_routine_select_random_marker_set(database, filepath):
    ...
    with sqlite3.connect(database) as con:
        ...
        for level in level_dict.keys():
            cur.execute(
                """SELECT genomeID from Genomes WHERE Phylum = ? LIMIT 5""", [level]
            )
            for row in cur:
