...
from hmsss.graft.taxonomy_cleaner import TaxonomyCleaner
...
class Getaxnseq:
    ...
    def read_taxtastic_taxonomy_and_seqinfo(self, taxonomy_io, seqinfo_io):
        ...
        for line in taxonomy_io:
            ...
            if expected_number_of_fields is None:
                ...
                raise Exception(
                    "Encountered error parsing taxonomy file, expected %i fields but found %i on line: %s"
        ...
        for i, line in enumerate(seqinfo_io):
            ...
            if len(splits) != 2:
                raise Exception(
                    "Bad formatting of seqinfo file on this line: %s" % line
                )
    ...
    def write_taxonomy_and_seqinfo_files(
        self, taxonomies, output_taxonomy_file, output_seqinfo_file
    ):
        ...
        for j, array in enumerate(first_pass_id_and_taxonomies):
            ...
            for i, tax in enumerate(taxonomy):
                if i == 0:
                    continue  # top levels don't have parents
                ...
                if tax in parents:
                    if parents[tax] != ancestry:
                        ...
                        if dup not in known_duplicates:
                            print(
                                " %s '%s' with multiple parents %s and %s"
        ...
        with open(output_taxonomy_file, "w") as seqout:
            ...
            for array in first_pass_id_and_taxonomies:
                ...
                for i, tax in enumerate(taxons):
                    line = self._taxonomy_line(
                        i, taxons[: (i + 1)], taxonomic_level_names
                    )
