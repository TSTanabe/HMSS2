...
def parallel_translation(directory: str, cores: int) -> None:
    ...
    logger.info(
        f"Found {len(NucleotideFastaFiles)} assemblies in nucleotide or ambiguous format for prodigal"
    )
...
def create_selfquery_file(options) -> None:
    ...
    with (
        open(options.self_query, "w") as outfile_query,
        open(options.self_seqs, "w") as outfile_seqs,
    ):
...
def deconcat(options) -> None:
    ...
    options.fasta_file_directory = (
        options.result_files_directory + "/deconcatenated_fasta_files"
    )
