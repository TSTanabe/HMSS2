...
def parse_arguments(arguments: list):
    ...
    def add_all_groups(parser, show_advanced: bool):
        ...
        inputdef.add_argument(
            '-f', dest='fasta_file_directory', type=myUtil.dir_path, default=None,
            metavar='<directory>', help='Directory to be searched'
        )
        inputdef.add_argument(
        ...
        inputdef.add_argument(
            ...
            metavar='<filepath>', help='Filepath to existing sqlite database'
        )    
        inputdef.add_argument(
        ...
        parameters.add_argument(
            '-cut_type', dest='threshold_type', type=int, default=1,
            metavar='<int>', choices=[1, 2, 3],
            help='Choice of cutoff: 1 optimized; 2 trusted; 3 noise' if show_advanced else argparse.SUPPRESS
        )
        parameters.add_argument(
            '-cut_score', dest='thrs_score', type=int, default=50,
            metavar='<int>', help='Global minimal score cutoff' if show_advanced else argparse.SUPPRESS
        )
        parameters.add_argument(
        ...
        parameters.add_argument(
            '-n', dest='name', type=str, default="project",
            metavar='<string>', help='Name new project' if show_advanced else argparse.SUPPRESS
        )
        parameters.add_argument(
        ...
        resources.add_argument(
            '-clean', dest='clean_reports', action='store_true',
            help='Clean up any pre-existing HMMreport file' if show_advanced else argparse.SUPPRESS
        )
        resources.add_argument(
            '-no_reports', dest='individual_reports', action='store_false',
        ...
        synteny.add_argument(
            '-chunks', dest='glob_chunks', type=int, default=5000,
            metavar='<int>', help='Chunk size for parsing results from glob before entering into database' if show_advanced else argparse.SUPPRESS
        )

        # Information on resources
        information = parser.add_argument_group("Information on resources")
        information.add_argument(
            '-stat_keywords', action='store_true',
            help='Print patterns for keyword naming' if show_advanced else argparse.SUPPRESS
        )
        information.add_argument(
            '-stat_csb', action='store_true',
            help='Print automatically found csbs' if show_advanced else argparse.SUPPRESS
        )
        information.add_argument(
            '-stat_genomes', action='store_true',
            help='Print taxonomy information from database' if show_advanced else argparse.SUPPRESS
        )

        # Collinear syntenic block prediction
        csb = parser.add_argument_group("Collinear syntenic block prediction")
        csb.add_argument(
            '-nt', dest='nucleotide_range', type=int, default=3500,
            metavar='<int>', help='Max. nucleotide distance to be considered synthenic genes' if show_advanced else argparse.SUPPRESS
        )
        csb.add_argument(
            '-insertions', dest='insertions', type=int, default=1,
            metavar='<int>', help='Max. insertions in a csb' if show_advanced else argparse.SUPPRESS
        )
        csb.add_argument(
            '-occurence', dest='occurence', type=int, default=2,
            metavar='<int>', help='Min. number occurences to be recognized as csb' if show_advanced else argparse.SUPPRESS
        )
        csb.add_argument(
            '-min_csb_size', dest='min_csb_size', type=int, default=4,
            metavar='<int>', help='Min. number of genes in a csb before recognized' if show_advanced else argparse.SUPPRESS
        )
        csb.add_argument(
            '-max_csb_size', dest='max_csb_size', type=int, default=50,
            metavar='<int>', help='Max. number of genes in a csb before recognized' if show_advanced else argparse.SUPPRESS
        )
        csb.add_argument(
            '-max_gene_repeats', dest='max_domain_repeats', type=int, default=4, metavar='<int>',
            help="Maximum number of repeated genes in a csb." if show_advanced else argparse.SUPPRESS
        )        
        csb.add_argument(
            '-jaccard', dest='jaccard', type=float, default=0.0,
            metavar='<float>', help='Acceptable dissimilarity in jaccard clustering [0.0-1.0]' if show_advanced else argparse.SUPPRESS
        )

        # Work step regulation
        flow = parser.add_argument_group("Work step regulation")
        flow.add_argument(
            '-redo_taxonomy', dest='redo_taxonomy', action='store_true',
            help='Redo the taxonomy assignment*. Default: False' if show_advanced else argparse.SUPPRESS
        )
        ...
        limiter.add_argument(
            '-dll', dest='dataset_limit_lineage', type=str, default=None,
            metavar='<string>',
            choices=['Superkingdom', 'Phylum', 'Class', 'Ordnung', 'Family', 'Genus', 'Species'],
            help='Taxonomy level [Superkingdom,Phylum,Class,Ordnung,Family,Genus,Species]' if show_advanced else argparse.SUPPRESS
        )
        limiter.add_argument(
            '-dlt', dest='dataset_limit_taxon', type=str, default=None,
            metavar='<string>', help='Taxonomic name e.g. Proteobacteria. Requires -dll' if show_advanced else argparse.SUPPRESS
        )
        limiter.add_argument(
            '-dlp', dest='dataset_limit_proteins', type=str, default=0,
            metavar='<list>', help='Limit fetch to genomes with <protein>' if show_advanced else argparse.SUPPRESS
        )
        limiter.add_argument(
            '-dlk', dest='dataset_limit_keywords', type=str, default=0,
            metavar='<list>', help='Limit fetch to genomes with <keyword>' if show_advanced else argparse.SUPPRESS
        )
        limiter.add_argument(
            '-dtd', dest='dataset_divide_sign', default='.', type=str,
            metavar='<string>',
            help='Separator for taxonomy information. The characters ";" ":" and "," cause strange behavior' if show_advanced else argparse.SUPPRESS
        )
        ...
        operators.add_argument(
            '-fl', dest='dataset_limit_lineage', type=str, metavar='<string>',
            choices=['Superkingdom', 'Phylum', 'Class', 'Ordnung', 'Family', 'Genus', 'Species'],
            help='Taxonomy level [Superkingdom,Phylum,Class,Ordnung,Family,Genus,Species]'
        )
        operators.add_argument(
            '-ft', dest='dataset_limit_taxon', type=str, metavar='<string>',
            help='Taxonomic name e.g. Proteobacteria. Requires -fl' if show_advanced else argparse.SUPPRESS
        )
        operators.add_argument(
            '-fg', nargs='+', dest='fetch_genomes', type=str, default=[],
            metavar='<list>', help='Select only genomes with these identifiers (whitespace separated)' if show_advanced else argparse.SUPPRESS
        )
        operators.add_argument(
            '-fd', nargs='+', dest='fetch_proteins', type=str, default=[],
            metavar='<list>', help='Select only proteins with these domains (whitespace separated)'
        )
        operators.add_argument(
            '-fc', nargs='+', dest='fetch_csbs', type=str, default=[],
            metavar='<list>', help='Select only csb encoding the given proteins (whitespace separated)'
        )
        operators.add_argument(
            '-fk', nargs='+', dest='fetch_keywords', type=str, default=[],
            metavar='<list>', help='Select only proteins in gene cluster with this keyword (whitespace separated)' if show_advanced else argparse.SUPPRESS
        )
        operators.add_argument(
            '-kc', dest='keywords_connector', type=str, default='OR',
            choices=['AND', 'OR'],
            help='Select cluster with keywords connected by AND or OR' if show_advanced else argparse.SUPPRESS
        )

        # Alignment and sequence file processing
        process = parser.add_argument_group("Alignment and sequence file processing")
        process.add_argument(
            '-merge_fasta', dest='merge_fasta', type=myUtil.dir_path,
            metavar='<directory>', help='Merges two or more sequence files with extention .faa without doublicates' if show_advanced else argparse.SUPPRESS
        )
        process.add_argument(
            '-filter_fasta', dest='filter_fasta', type=str,
            metavar='<file> <int> <int>', help='Filter fasta file by length upper and lower limit' if show_advanced else argparse.SUPPRESS
        )
        process.add_argument(
            '-concat_alignment', dest='concat_alignment', type=myUtil.dir_path,
        ...
        process.add_argument(
            '-add_genomic_context', dest='add_genomic_context', type=myUtil.file_path,
            metavar='<file>',
            help='Adds genomic context to sequences from fasta file, requires -db with taxonomy' if show_advanced else argparse.SUPPRESS
        )
        process.add_argument(
            '-create_type_range_dataset', dest='create_type_range_dataset',
            type=myUtil.file_path, metavar='<file>',
            help='Create protein type range dataset from sequences fasta file, requires -db with taxonomy' if show_advanced else argparse.SUPPRESS
        )
        process.add_argument(
            '-create_gene_cluster_dataset', dest='create_gene_cluster_dataset',
            type=myUtil.file_path, metavar='<file>',
            help='Create gene cluster dataset from sequences fasta file, requires -db with taxonomy' if show_advanced else argparse.SUPPRESS
        )
        process.add_argument(
            '-aln_gaps', dest='gaps', action='store_true',
            help='When concating alignments add gaps for missing sequences' if show_advanced else argparse.SUPPRESS
        )
