...
def fetch_all_genomes(database: str) -> List[str]:
    query = """
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Train positive-only Bernoulli support "
            "models using a single PAM fetch "
            "per domain."
        )
    )
    ...
    parser.add_argument(
        "--models-jsonl",
        default="positive_support_models.jsonl",
    )

    parser.add_argument(
        "--evaluation-report",
        default="positive_support_test_evaluation.tsv",
    )
    ...
    for i, domain in enumerate(domains, 1):
        ...
        try:
            ...
            print(
                f"    [INFO] "
                f"Fetching combined PAM for "
                f"{len(all_needed_genomes):,} genomes"
            )

            all_pam = fetch_pam_for_genomes(
                args.database,
                all_needed_genomes,
                args.chunk_size,
                args.cores,
