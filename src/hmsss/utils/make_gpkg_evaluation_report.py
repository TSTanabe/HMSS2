...
def _jitter_x(
    ...
    *,
    ...
    - width: volle Breite (Punkte liegen in [x_center - width/2, x_center + width/2])
    ...
    return x_center + rng.uniform(-width / 2.0, width / 2.0, size=n)
...
def plot_single_metric_boxplot(
    ...
    *,
    ...
    if n == 1:
        ...
        width = 0.08  # volle Breite des Jitters
        rng = np.random.default_rng(42)  # Seed für Reproduzierbarkeit
        xs = 1.0 + rng.uniform(-width / 2.0, width / 2.0, size=n)
...
def plot_rank_fraction_boxplots(
    ...
    *,
    ...
    if not data:
        print(
            f"[WARN] No valid data for {numerator_prefix}/{denominator}. Skipping plot."
        )
        return None
    ...
    for i, vals in enumerate(data, start=1):
        xs = _jitter_x(
            ...
            len(vals),
            width=0.18,
            seed=i,  # stabil pro Box, aber verschieden zwischen Boxen
        )
        ax.scatter(
            xs,
            vals,
...
def _rates(tp, fp, fn, tn):
    # robuste Division (falls mal 0)
    fpr = fp / (fp + tn) if (fp + tn) else float("nan")
    fnr = fn / (tp + fn) if (tp + fn) else float("nan")
    return fpr, fnr


def plot_fpr_fnr(points, title, out_png=None):
    """
    ...
    # Referenzlinie: FNR = FPR
    ax.plot([0, 1], [0, 1], linewidth=1)

    # optional: Labels (nur wenn wenige Punkte)
    if len(labels) <= 20:
...
def make_all_evaluation_plots(
    ...
    for key, title, ylabel in overall_specs:
        ...
        if p is not None:
            written.append(p)
    plot_fpr_fnr(
        points=collect_overall_confusion(eval_stat_list),
        title="Overall: FPR vs FNR",
        out_png=out_dir / "overall_fpr_fnr.png",
    )
    # --- Placement metrics ---
    placement_specs: List[Tuple[str, str, str]] = [
        (
            "placement_balanced_accuracy",
            "Placement balanced accuracy",
            "placement balanced accuracy",
        ),
    ...
    plot_fpr_fnr(
        points=collect_placement_confusion(eval_stat_list),
        title="Placement: FPR vs FNR",
        out_png=out_dir / "placement_fpr_fnr.png",
    )
    # --- Rank fraction plots ---
    rank_specs: List[Tuple[str, str, str, str, str]] = [
        ...
        (
            ...
            "TP_frags",
            "rank",
            "Assigned fraction per rank (assigned / TP_frags)",
            "assigned / TP_frags",
        ),
        (
            "correct",
            "TP_frags",
            "rank",
            "Correct fraction per rank (correct / TP_frags)",
            "correct / TP_frags",
        ),
        (
            "correct",
            "assigned",
            "rank",
            "Correct among assigned per rank (correct / assigned)",
            "correct / assigned",
        ),
        (
            "wrong",
            "assigned",
            "rank",
            "Wrong among assigned per rank (wrong / assigned)",
            "wrong / assigned",
        ),
    ]
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument(
        "--show-path", action="store_true", help="Include relative path column"
    )
