...
class HouseKeeping:
    ...
    def file_basename(self, file):
        ...
        if suffix in valid_extensions:
            ...
            logging.error(
                "For trees, please provide a file with one of the \
following extensions: %s"
                % " ".join(valid_extensions.keys())
            )
    ...
    def set_euk_hmm(self, args):
        ...
        if hasattr(args, "euk_hmm_file"):
            ...
            setattr(
                ...
                "euk_hmm_file",
                os.path.join(
                    os.path.dirname(inspect.stack()[-1][1]), "..", "share", "18S.hmm"
                ),
            )
        else:
    ...
    def setpipe(self, hmm):
        ...
        hmm_read = [
            x.rstrip()
            for x in open(hmm, "r").readlines()
            if x.startswith("ALPH") or x.startswith("TC")
        ]
        if len(hmm_read) > 2:
    ...
    def make_working_directory(self, directory_path, force):
        if force:
            ...
            try:
                ...
                logging.error(
                    "Directory %s already exists. Exiting to prevent over-writing"
                    % directory_path
                )
                raise Exception(
                    "Directory %s already exists. Exiting to prevent over-writing"
    ...
    def parameter_checks(self, args):
        ...
        if args.subparser_name == "graft":
            ...
            if float(args.placements_cutoff) < float(0.5) or float(
                args.placements_cutoff
            ) > float(1.0):
                logging.info(
                    "Please specify a confidence level (-d) between 0.5 and 1.0! Found: %s"
                    % args.placements_cutoff
                )
                exit(1)
            ...
            if args.interleaved and args.forward:
                logging.info(
                    "Please specify either reads with one of"
                    "--forward or --interleaved, not both"
                )
                exit(1)
            if (not args.interleaved) and (not args.forward):
                logging.info(
                    "Please specify either reads with either--forward or --interleaved"
                )
                exit(1)
            if args.interleaved:
            ...
            if args.reverse:
                if len(args.forward) != len(args.reverse):
                    logging.error(
                        "Confusing input. There appears to be different numbers of forward and reverse files specified"
                    )
        ...
        if args.graftm_package:
            ...
            if args.search_method == self.HMMSEARCH_SEARCH_METHOD:
                setattr(
                    args,
                    "search_hmm_files",
                    [x.rstrip() for x in open(args.search_hmm_list_file).readlines()],
                )
                if not hasattr(args, "aln_hmm_file"):
                    if not args.search_only:
                        raise Exception(
