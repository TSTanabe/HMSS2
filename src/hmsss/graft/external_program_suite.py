...
class ExternalProgramSuite:
    ...
    programs_to_possibilities = {
        _FASTTREE: ["FastTreeMP", "fasttree", "fasttreeMP", "FastTree"]
    }
    ...
    def __init__(self, program_list):
        ...
        for program in program_list:
            if program in ExternalProgramSuite.programs_to_possibilities:
                check = [
                    p
                    for p in ExternalProgramSuite.programs_to_possibilities[program]
                    if shutil.which(p)
                ]
                if len(check) > 1:
                    logging.warning(
                        "Program found with multiple commands: %s. \
Arbitrarily selecting %s"
                        % (" ".join(program_list), check[0])
                    )

                for key, item in ExternalProgramSuite.programs_to_possibilities.items():
                    if program in item:
                        program = key
        ...
        if any(uninstalled_programs):
            ...
            for program in sorted(uninstalled_programs):
                ...
                logging.error(
                    "\t%25s\t%s"
                    % (print_name, ExternalProgramSuite.package_urls[package_name])
                )
            exit(1)
