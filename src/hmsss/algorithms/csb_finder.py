...
class Cluster:
    ...
    def add_keyword(
        ...
        additional_elements: Optional[Set[str]] = None,
        keyword_id: Any = ".",
    ) -> None:
        missing = missing or []
...
class Keyword:
    ...
    def __init__(
        ...
        csb: str = ".",
        missing: Optional[Set[str]] = None,
        additional: Optional[Set[str]] = None,
        keyword_id: Any = ".",
    ) -> None:
        if missing is None:
            missing = set()
        ...
        with open(filepath, "r") as reader:
            for line_num, line in enumerate(reader, start=1):
                ...
                try:
                    ...
                    logger.warn(
                        f"Skipping unrecognized pattern on line {line_num}: {line}"
                    )
...
def find_syntenic_blocks(
    ...
    protein_id_list = sorted(
        protein_dict,
        key=lambda x: (protein_dict[x].gene_contig, protein_dict[x].gene_start),
    )
    cluster_id_number = 1
    ...
    for index, elem in enumerate(protein_id_list):
        if index - 1 >= 0:
            ...
            if (
                prev_protein.gene_contig == curr_protein.gene_contig
                and curr_protein.gene_start - prev_protein.gene_end <= distance
            ):
                if new_sb:
                    new_sb = 0
                    cluster.add_gene(
                        prev_el_protein_id,
                        prev_protein.get_domains(),
                        prev_protein.gene_start,
                        prev_protein.gene_end,
                    )
                    prev_protein.clusterID = cluster.clusterID
                    cluster.add_gene(
                        curr_el_protein_id,
                        curr_protein.get_domains(),
                        curr_protein.gene_start,
                        curr_protein.gene_end,
                    )
