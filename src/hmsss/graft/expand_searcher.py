...
class ExpandSearcher:
    ...
    def __init__(self, **kwargs):
        ...
        if graftm_package:
            ...
            self.unaligned_sequence_database = (
                graftm_package.unaligned_sequence_database_path()
            )
    ...
    def generate_expand_search_database_from_contigs(
        self, contig_files, output_database_file, search_method
    ):
        ...
        if search_method == self.DIAMOND_SEARCH_METHOD:
            if (
                self.diamond_database == None
                or self.unaligned_sequence_database == None
            ):
                logging.warning(
        ...
        with tempfile.NamedTemporaryFile(prefix="graftm_expand_search_orfs") as orfs:
            ...
            for contig_file in contig_files:
                ...
                with tempfile.NamedTemporaryFile(
                    prefix="graftm_expand_search"
                ) as hit_reads_orfs_fasta:
                    # search and extract matching ORFs
                    with tempfile.NamedTemporaryFile(
                        prefix="graftm_expand_search2"
                    ) as hmmsearch_output_table:
                        with tempfile.NamedTemporaryFile(
                            prefix="graftm_expand_search3"
                        ) as hit_reads_fasta:
                            ss.search_and_extract_orfs_matching_protein_database(
                                ...
                                (
                                    self.diamond_database
                                    if self.diamond_database
                                    else None
                                ),
            ...
            with tempfile.NamedTemporaryFile(prefix="graftm_expand_search_aln") as aln:
                if search_method == self.HMM_SEARCH_METHOD:
                    ...
                    if (
                        len(seqio.read_fasta_file(orfs.name)) <= 1
                    ):  # Just to build on this, you need to check if there is > 1 hit
