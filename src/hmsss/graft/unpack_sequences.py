...
class UnpackRawReads:
    ...
    def __init__(self, read_file, known_sequence_type=None, interleaved=False):
        ...
        """
        logging.debug(
            "Loading %s, type %s, interleaved %s",
            read_file,
            known_sequence_type,
            interleaved,
        )
    ...
    def is_zcattable(self):
        return self.guess_sequence_input_file_format(self.read_file) in (
            self.FORMAT_FASTA_GZ,
            self.FORMAT_FASTQ_GZ,
        )

    def basename(self):
        """Return the name of the file with the '.fasta' or 'fq.gz' etc
        removed"""
        return os.path.basename(self.read_file)[
            : -len(self._get_extension(self.read_file))
        ]

    def _get_extension(self, sequence_file_path):
        for ext in list(self._EXTENSION_TO_FILE_TYPE.keys()):
            if sequence_file_path.endswith(ext):
                return ext
        raise self.UnexpectedFileFormatException(
            "Unable to guess file format of sequence file: %s" % sequence_file_path
        )
    ...
    def command_line(self):
        ...
        if file_format == self.FORMAT_FASTA:
            ...
            cmd = (
                """zcat '%s' | awk '{print ">" substr($0,2);getline;print;getline;getline}' -"""
                % (self.read_file)
            )
            ...
            cmd = (
                """awk '{print ">" substr($0,2);getline;print;getline;getline}' '%s'"""
                % (self.read_file)
            )
        if self.interleaved:
