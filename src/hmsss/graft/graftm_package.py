...
from hmsss.graft.getaxnseq import Getaxnseq

logging = get_logger(__name__)


class InsufficientGraftMPackageException(Exception):
    pass
...
class GraftMPackage:
    ...
    def acquire(graftm_package_path):
        ...
        with open(
            os.path.join(graftm_package_path, GraftMPackage._CONTENTS_FILE_NAME)
        ) as f:
            contents_hash = json.load(f)

        v = contents_hash[GraftMPackage.VERSION_KEY]
        logging.debug(
            "Loading version %i GraftM package: %s" % (v, graftm_package_path)
        )
        if v == 2:
    ...
    def check_universal_keys(self, version):
        ...
        try:
            ...
            raise InsufficientGraftMPackageException(
                "No version information in graftm package"
            )
        if v != version:
            raise InsufficientGraftMPackageException("Bad version: %s" % v)
...
class GraftMPackageVersion2(GraftMPackage):
    ...
    def search_hmm_paths(self):
        return [
            os.path.join(self._base_directory, x)
            for x in self._contents_hash[GraftMPackage.SEARCH_HMM_KEY]
        ]

    def alignment_hmm_path(self):
        return os.path.join(
            self._base_directory, self._contents_hash[GraftMPackage.ALIGNMENT_HMM_KEY]
        )

    def alignment_fasta_path(self):
        return os.path.join(
            self.reference_package_path(), self._refpkg_contents()["files"]["aln_fasta"]
        )
    ...
    def _refpkg_contents(self):
        with open(os.path.join(self.reference_package_path(), "CONTENTS.json")) as f:
            return json.load(f)

    def taxtastic_seqinfo_path(self):
        return os.path.join(
            self.reference_package_path(), self._refpkg_contents()["files"]["seq_info"]
        )

    def taxtastic_taxonomy_path(self):
        return os.path.join(
            self.reference_package_path(), self._refpkg_contents()["files"]["taxonomy"]
        )

    def reference_package_tree_path(self):
        return os.path.join(
            self.reference_package_path(), self._refpkg_contents()["files"]["tree"]
        )
    ...
    def compile(
        ...
        if os.path.exists(output_package_path):
            raise Exception(
                "Not writing new GraftM package to already existing file/directory with name %s"
                % output_package_path
            )
        ...
        if diamond_database_file:
            ...
            shutil.copyfile(
                diamond_database_file,
                os.path.join(output_package_path, diamond_database_file_in_gpkg),
            )
        ...
        if search_hmm_files:
            ...
            if hmm_file_in_gpkg in search_hmm_files_in_gpkg_names:
                raise Exception(
                    "Search and align HMMs must all have different basenames to create a new gpkg"
                )
            if len(set(search_hmm_files_in_gpkg_names)) != len(
                search_hmm_files_in_gpkg_names
            ):
                raise Exception(
                    "Search HMMs must have different basenames to create a new gpkg"
                )
            for i, search_hmm in enumerate(search_hmm_files):
                shutil.copyfile(
                    search_hmm,
                    os.path.join(
                        output_package_path, search_hmm_files_in_gpkg_names[i]
                    ),
                )
        else:
        ...
        contents = {
            GraftMPackage.VERSION_KEY: GraftMPackageVersion2.version,
        ...
        with open(
            os.path.join(output_package_path, GraftMPackage._CONTENTS_FILE_NAME), "w"
        ) as f:
...
class GraftMPackageVersion3(GraftMPackageVersion2):
    ...
    def create_diamond_db(self):
        ...
        cmd = "diamond makedb --in '%s' -d '%s'" % (
            self.unaligned_sequence_database_path(),
            base,
        )
    ...
    def is_protein_package(self):
        ...
        if not hasattr(self, "_is_protein_package"):
            self._is_protein_package = GraftMPackageVersion3.graftm_package_is_protein(
                self
            )
    ...
    def compile(
        ...
        diamond_database_file,
        max_range,
        unaligned_sequence_database=None,
        trusted_cutoff=False,
        search_hmm_files=None,
    ):
        """Create a new GraftM package with the given inputs. Any files
        ...
        if unaligned_sequence_database:
            unaligned_sequence_database_in_gpkg = os.path.join(
                output_package_path, os.path.basename(unaligned_sequence_database)
            )
            shutil.copyfile(
                unaligned_sequence_database, unaligned_sequence_database_in_gpkg
            )
        ...
        contents = {
            GraftMPackage.VERSION_KEY: GraftMPackageVersion3.version,
            GraftMPackage.ALIGNMENT_HMM_KEY: hmm_file_in_gpkg,
            GraftMPackage.SEARCH_HMM_KEY: search_hmm_files_in_gpkg_names,
            GraftMPackage.REFERENCE_PACKAGE_KEY: refpkg_in_gpkg,
            GraftMPackage.HMM_TRUSTED_CUTOFF_KEY: trusted_cutoff,
            ...
            GraftMPackage.UNALIGNED_SEQUENCE_DATABASE_KEY: (
                os.path.basename(unaligned_sequence_database)
                if unaligned_sequence_database
                else None
            ),
